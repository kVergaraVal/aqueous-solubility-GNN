...
def train(cv_index, epoch, train_loader, empty_solvsys, model, loss_fn1, loss_fn2, optimizer, pinn_lambda=0, batch_adding=False, wandb_logs=False):
    ...
    # Logging
    print("[Stage {}]: Epoch {} finished at time={:.3f} with loss={:.3f} lossPred={:.3f} loss1={:.3f} loss2={:.3f} lossGD={:.6f}".format(
    stage, epoch, batch_time.sum, loss_accum.avg, loss_pred_accum.avg, loss1_accum.avg, loss2_accum.avg, loss_gd_grad_accum.avg))
    if wandb_logs:
        wandb.log({
                f"epoch": epoch,
...
def main(hyperparameter):
    ...
    save_add = f"_{data}_split-{data_split_mode}_nums{n_splits}-{model_type}_pinn_l{pinn_lambda}_se{pinn_start_epoch}_dropout{mlp_dropout_rate}_act{mlp_activation}_encAct{enc_activation}_nhl{mlp_num_hid_layers}_batchA{batch_adding}_lrsched{use_lr_scheduler}_epochs{epochs}_lr{lr}_batchsize{batch_size}_messagepassingsteps{num_step_message_passing}"
    if data_split_mode == "comp_extra":
        save_add = f"_{data}_split-{data_split_mode}_comp{comp_range}-{model_type}_pinn_l{pinn_lambda}_se{pinn_start_epoch}_dropout{mlp_dropout_rate}_act{mlp_activation}_encAct{enc_activation}_nhl{mlp_num_hid_layers}_batchA{batch_adding}_lrsched{use_lr_scheduler}_epochs{epochs}_lr{lr}_batchsize{batch_size}_messagepassingsteps{num_step_message_passing}"
    ...
    for train_indices, val_indices in zip(train_indices_splits, val_indices_splits):
        ...
        if model_type == "SolvGNN":
            ...
            model = gegnn_binary(in_dim=74, hidden_dim=hidden_dim, n_classes=1, mlp_dropout_rate=mlp_dropout_rate, mlp_activation=mlp_activation, mlp_num_hid_layers=mlp_num_hid_layers, num_step_message_passing=num_step_message_passing).cuda()
...
if __name__ == '__main__':
    ...
    # Model architecture
    parser.add_argument('--model_type', default="GEGNN", type=str) # options: SolvGNN, SolvGNNxMLP, MCM_multiMLP, GEGNN
    parser.add_argument('--batch_size', default=32, type=int) ##ORIGINAL: 100 // checkpoint: 512
    ...
    parser.add_argument('--mlp_num_hid_layers', default=0, type=int) ##ORIGINAL: 2 // checkpoint: 0? 
    ...
    # Training
    parser.add_argument('--batch_adding', default="False", type=str) #ORIGINAL: True // #checkpoint: False
    parser.add_argument('--lr', default=5*1e-3, type=float)                   #ORIGINAL: 1e-3
