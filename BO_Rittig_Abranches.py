...
class solute_dataset_water_SP:
    def __init__(self, input_file_path = DATASET_PATH,
...
class SigmaProfileGCN(nn.Module):
    ...
    def forward(self, molecule_data, device):
        ...
        with graph.local_scope():
            ...
            h1_temp = AF.relu(self.conv1(graph, h1))
            h1_temp = AF.relu(self.conv2(graph, h1_temp))
            h1_temp = AF.relu(self.conv3(graph, h1_temp))
            graph.ndata['h'] = h1_temp

            hg = h1_temp
            output = AF.relu(self.classify(hg))
...
class Rittig_Abranches_variant(nn.Module):
    def __init__(self, in_dim_rit, in_dim_abr, n_classes, sigma_profile_length, weight_init_fn,
                    ...
                    device, 
                    sigma_profile_model_path=SIGMA_PROFILE_MODEL_PATH):
        super(Rittig_Abranches_variant, self).__init__()
...
def get_CNN_num_out_features(n_conv_filters, kernel_size, stride, pool_size):
    pass
...
class Rittig_Abranches(nn.Module):
    ...
    ## Abranches Module
        ...
        conv_1_param = [n_conv_filters, kernel_size, stride]       #Channels (filters), kernel, stride
        self.pool_1_size = pool_size
        self.pool_1_stride = self.pool_1_size
        conv_2_param = [n_conv_filters, kernel_size, stride] 
        ...
        #self.dense_1 = nn.Linear(in_features=get_CNN_num_out_features(sigma_profile_length, n_conv_filters, kernel_size, stride, pool_size), out_features=dense_1_nodes)
        ...
        if weight_init_fn == torch.nn.init.uniform_:
    ...
    # Rittig Module
        ...
        self.global_conv1 = MPNNconv(node_in_feats=gcn_dim,
                                     ...
                                     activation=mpnn_activation)
        self.mlp_activation = get_activation(mlp_activation)
        self.mfp_trans = nn.Linear(mpnn_dim+SP_dim, mlp_dim)
        self.mlp2 = nn.Linear(mlp_dim, mlp_dim)
        self.mlp3 = nn.Linear(mlp_dim, mlp_dim)
        self.output_layer = nn.Linear(mlp_dim, n_classes)
    ...
    # Abranches Module
        ...
        x = AF.silu(self.conv1d_1(sp))
        padded_x = AF.pad(x, padding_pool(self.pool_1_size), mode='replicate')
        x = self.pool_1(padded_x)

        x = AF.silu(self.conv1d_2(x))
        padded_x = AF.pad(x, padding_pool(self.pool_2_size), mode='replicate')
        x = self.pool_2(padded_x)

        #print(f'x.shape is: {x.shape}')
        flattened_x = x.view(x.shape[0], -1)
        #print(f'flattened_x is {flattened_x.shape}')

        x = AF.silu(self.dense_1(flattened_x))
        x = AF.silu(self.dense_2(x))
    ...
    # Rittig Module
        ...
        with g1.local_scope():
            with g2.local_scope():
                ...
                # MLP
                output = self.mlp_activation(self.mfp_trans(hg_temp))
                output = self.mlp_activation(self.mlp2(output))
                output = self.mlp_activation(self.mlp3(output))
                output = self.output_layer(output) 
...
def train_and_evaluate(trial, gcn_dim, mpnn_dim, mlp_dim, n_conv_filters, kernel_size, pool_size, SP_dim, learning_rate, batch_size, l2_coef, gpu_id):
    """
    ...
    for train_indices, val_indices in zip(train_indices_splits, val_indices_splits):
        ...
        mlp_num_hid_layers = 3
        mlp_activation = 'softplus'

        # initialize model
        if MODEL_TYPE == "Rittig_Abranches":
            model = Rittig_Abranches(in_dim_rit=74, in_dim_abr=50, n_classes=1, sigma_profile_length=51, weight_init_fn=weight_init_fn,
        ...
        for epoch in range(EPOCHS):
            ...
            if epoch % 4 == 0 or epoch == 50 or epoch == 100:
...
def objective(trial):
    ...
    """
    #Architecture
    GCN_dim = trial.suggest_int("GCN_dimension", 1, 300)
    MPNN_dim = trial.suggest_int("MPNN_dimension", 1, 300)
    MLP_dim = trial.suggest_int("MLP_dimension", 1, 300)
    n_conv_filters = trial.suggest_int('num_conv_filters', 1, 32)
    kernel_size = trial.suggest_int('kernel_size', 1, 10)
    pool_size = trial.suggest_int('pool_size', 1, 10)
    SP_dim = trial.suggest_int('SP_dense_dimension', 1, 32)
    ...
    #try:
    #    loss = train_and_evaluate(trial, GCN_dim, MPNN_dim, MLP_dim, n_conv_filters, kernel_size, pool_size, SP_dim, learning_rate, batch_size, l2_coef, gpu_id)
    #except:
    #    loss = None
...
study = optuna.create_study(
    study_name="Test_Rittig_Abranches",
    direction="minimize",
    storage="sqlite:////home/kevinvergara/Physics-informed-Thermodynamics/TRIALS_BO_rittig_abranches_database.db",  # Checkpointing database
    load_if_exists=True,
    sampler=sampler
)
...
#plot_param_importances.write_image("fig1.svg")