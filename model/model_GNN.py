...
class Qin_ternary(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes, mlp_activation):
        ...
        with g1.local_scope():
            with g2.local_scope():
                with g3.local_scope():
                    ...
                    # SLP
                    hg1_temp = self.mlp_activation(self.mfp_trans(torch.cat((hg[0:len(hg)//3,:], solv1x[:, None]), axis=1)))
                    hg2_temp = self.mlp_activation(self.mfp_trans(torch.cat((hg[len(hg)//3:2*len(hg)//3,:], 1-solv1x[:, None]), axis=1)))
                    hg3_temp = self.mlp_activation(self.mfp_trans(torch.cat((hg[2*len(hg)//3:,:], solv1x[:, None]), axis=1)))
                    # Pooling
                    hg_temp = (hg1_temp + hg2_temp + hg3_temp) / 3

                    #MLP
                    output = F.relu(self.classify1(hg_temp))
                    ...
                    gamma = torch.cat((gamma_1.unsqueeze(-1), gamma_2.unsqueeze(-1)),axis=1)
                    gamma_3 = gamma_3.squeeze(-1)
                    gamma = torch.cat((gamma, gamma_3.unsqueeze(-1)), axis=1)  
...
class Qin_ternary_optimized(nn.Module):
    def __init__(self, in_dim, gcn_dim, mpnn_dim, mlp_dim, n_classes, mlp_activation):
        super(Qin_ternary_optimized, self).__init__()
        ...
        self.global_conv1 = MPNNconv(node_in_feats=gcn_dim,
                                     ...
                                     num_step_message_passing=1)
        self.mlp_activation = get_activation(mlp_activation)
        self.mfp_trans = nn.Linear(mpnn_dim+1, mlp_dim+1)
        self.classify1 = nn.Linear(mlp_dim+1, mlp_dim)
        ...
        with g1.local_scope():
            with g2.local_scope():
                with g3.local_scope():
                    ...
                    hg1_temp = self.mlp_activation(self.mfp_trans(torch.cat((hg[0:len(hg)//3,:], solv1x[:, None]), axis=1)))
                    hg2_temp = self.mlp_activation(self.mfp_trans(torch.cat((hg[len(hg)//3:2*len(hg)//3,:], 1-solv1x[:, None]), axis=1)))
                    hg3_temp = self.mlp_activation(self.mfp_trans(torch.cat((hg[2*len(hg)//3:,:], frac_0[:, None]), axis=1)))
                    # Pooling
                    hg_temp = (hg1_temp + hg2_temp + hg3_temp) / 3
