...
MODEL_TYPE = "Rittig_Abranches_variant"
...
class Rittig_Abranches(nn.Module):
    ...
    ## Abranches Module
        ...
        self.dense_1 = nn.Linear(in_features=get_CNN_num_out_features(n_conv_filters, kernel_size, kernel_size//2, pool_size), out_features=dense_1_nodes)
...
def train_and_evaluate(trial, gcn_dim, mpnn_dim, mlp_dim, learning_rate, batch_size, l2_coef, use_scheduler, gpu_id):
    """
    ...
    for train_indices, val_indices in zip(train_indices_splits, val_indices_splits):
        ...
        # initialize model
        if MODEL_TYPE == "Rittig_Abranches_variant":
            model = Rittig_Abranches_variant(in_dim_rit=74, in_dim_abr=50, n_classes=1, sigma_profile_length=51, weight_init_fn=weight_init_fn,
...
def objective(trial):
    ...
    loss = train_and_evaluate(trial, GCN_dim, MPNN_dim, MLP_dim, learning_rate, batch_size, l2_coef, use_scheduler, gpu_id)
...
study = optuna.create_study(
    study_name="Test_Rittig_Abranches_variant",
    direction="minimize",
    storage="sqlite:////home/kevinvergara/Physics-informed-Thermodynamics/TRIALS_BO_rittig_abranches_database_variant_fittingparam_gcn282_mpnn198_mlp119.db",  # Checkpointing database
    load_if_exists=True,
    sampler=sampler
)
